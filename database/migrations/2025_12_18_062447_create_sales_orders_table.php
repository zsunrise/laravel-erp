...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('sales_orders', function (Blueprint $table) {
            $table->id();
            $table->string('order_no')->unique()->comment('销售单号');
            $table->foreignId('customer_id')->constrained('customers')->onDelete('restrict');
            $table->foreignId('warehouse_id')->constrained('warehouses')->onDelete('restrict');
            $table->date('order_date')->comment('订单日期');
            $table->date('delivery_date')->nullable()->comment('交货日期');
            $table->string('status')->default('draft')->comment('状态：draft-草稿，pending-待审核，approved-已审核，partial-部分出库，completed-已完成，cancelled-已取消');
